public class ImageRotator {
	public static void main(String[] args) {
		...
		/**
		 * Given an image represented by an matrix, write a method to rotate the image by 90 degrees.
		 * 
		 */
		...
		String ImageI[][]=new String[Image[0][0].length()][Image.length];
		
		for (int i = 0; i < Image.length; i++) {
			String line = Image[i][0];
			for (int j = 0; j < line.length(); j++) {
				ImageI[j][i]= line.substring(j, j+1);
			}
		}
		
		for (int i = 0; i < ImageI.length; i++) {
			for (int j = 0; j < ImageI[i].length; j++) {
				System.out.print(ImageI[i][j]);
			}
			System.out.println();
		}
	}
